<script lang="ts">
import {defineComponent, ref, h, onMounted, Ref, onBeforeMount} from 'vue';
...
import { saleReportList } from "/@/api/demo/system";
import { useRouter } from "vue-router";
import { regionLabelMap } from "/@/enums/menuEnum";
import {isDevMode, isTestMode} from "/@/utils/env";
...
export default defineComponent({
  ...
  setup() {
    ...
    const pageCode = ref(0) as Ref<number | string>;
    ...
    const dateRange = ref([moment().startOf('months'), moment().subtract(1, 'days')]);
    ...
    const [registerForm, { setProps }] = useForm({
      trackParams: { opType: 3, click: true, opDesc: '查询按钮' },
      labelWidth: 100,
      ...
      resetButtonOptions: {
        preIcon: 'arrow-right|svg',
      },
      isChangeActionButton: true,
    });
    ...
    async function handleReset() {
      dateRange.value = [moment().startOf('months'), moment().subtract(1, 'days')];
      columns = [
        {
          title: '当月达成情况',
          dataIndex: 'current_month',
    ...
    function getRowSpan1(index) {
      ...
      const regionNameMap = isTestMode() || isDevMode() ?
        {
        '1013': xiaoShouNum,
      ...
      } else if (length >= 2 && index > numMapArr[0].count && index <= numMapArr[0].count + numMapArr[1].count - 1) {
        return 0;
      } else if (length >= 3 && index === numMapArr[0].count + numMapArr[1].count) {
        return numMapArr[2].count;
      } else if (length >= 3 && index > numMapArr[0].count + numMapArr[1].count && index <= numMapArr[0].count + numMapArr[1].count + numMapArr[2].count - 1) {
        return 0;
      } else if (length >= 4 && index === numMapArr[0].count + numMapArr[1].count + numMapArr[2].count) {
        return numMapArr[3].count;
      } else if (length >= 4 && index > numMapArr[0].count + numMapArr[1].count + numMapArr[2].count && index <= numMapArr[0].count + numMapArr[1].count + numMapArr[2].count + numMapArr[3].count - 1) {
        return 0;
      } else if (length >= 5 && index === numMapArr[0].count + numMapArr[1].count + numMapArr[2].count + numMapArr[3].count) {
        return numMapArr[4].count;
      } else if (length >= 5 && index > numMapArr[0].count + numMapArr[1].count + numMapArr[2].count + numMapArr[3].count && index <= numMapArr[0].count + numMapArr[1].count + numMapArr[2].count + numMapArr[3].count + numMapArr[4].count - 1) {
        return 0;
      } else if (length >= 6 && index === numMapArr[0].count + numMapArr[1].count + numMapArr[2].count + numMapArr[3].count + numMapArr[4].count) {
        return numMapArr[5].count;
      } else if (length >= 6 && index > numMapArr[0].count + numMapArr[1].count + numMapArr[2].count + numMapArr[3].count + numMapArr[4].count && index <= numMapArr[0].count + numMapArr[1].count + numMapArr[2].count + numMapArr[3].count + numMapArr[4].count + numMapArr[5].count - 1) {
        return 0;
      } else if (length >= 7 && index === numMapArr[0].count + numMapArr[1].count + numMapArr[2].count + numMapArr[3].count + numMapArr[4].count + numMapArr[5].count) {
        return numMapArr[6].count;
      } else if (length >= 7 && index > numMapArr[0].count + numMapArr[1].count + numMapArr[2].count + numMapArr[3].count + numMapArr[4].count + numMapArr[5].count && index <= numMapArr[0].count + numMapArr[1].count + numMapArr[2].count + numMapArr[3].count + numMapArr[4].count + numMapArr[5].count + numMapArr[6].count - 1) {
        return 0;
      } else if (length >= 8 && index === numMapArr[0].count + numMapArr[1].count + numMapArr[2].count + numMapArr[3].count + numMapArr[4].count + numMapArr[5].count + numMapArr[6].count) {
        return numMapArr[7].count;
      } else if (length >= 8 && index > numMapArr[0].count + numMapArr[1].count + numMapArr[2].count + numMapArr[3].count + numMapArr[4].count + numMapArr[5].count + numMapArr[6].count && index <= numMapArr[0].count + numMapArr[1].count + numMapArr[2].count + numMapArr[3].count + numMapArr[4].count + numMapArr[5].count + numMapArr[6].count + numMapArr[7].count - 1) {
        return 0;
      }
    }
    ...
    async function handleSubmit() {
      if (!dateRange.value[0] || !dateRange.value[1]) {
        createMessage.info('开始日期和截至日期不能为空！');
        return;
      }
      await loadList();
    }

    function startDayChange(date) {
      dateRange.value[0] = date;
    }

    function endDayChange(date) {
      dateRange.value[1] = date;
    }

    // 仅可选择从2020年1月1日 至 昨日
    function disabledStartDayDate(current) {
      const endDay = dateRange.value[1];
      return current
        && current > moment(endDay).subtract(1, 'days')
        || current < moment('2020-01-01')
        || current < moment(endDay).startOf('months');
    };

    function disabledEndDayDate(current) {
      const startDay = dateRange.value[0];
      return current
        && current < moment(startDay).add(1, 'days')
        || current > moment().subtract(1, 'days')
        || current > moment(startDay).endOf('months');
    }
    ...
    async function loadList() {
      ...
      try {
        ...
        setLoading(true);
        const res = await saleReportList(params);
        const { saleDetailVOList, weekIntervalVO } = res;
        for (let i = 11; i <= 15; i++) {
          columns[4]!.children[i]!.title = (weekIntervalVO[`intervalWeek${i - 10}`] ? weekIntervalVO[`intervalWeek${i - 10}`] : '') + `（第${toChineseNum(i - 10)}周）`
        }
        columns[5].title = `上月业绩情况（${weekIntervalVO.intervalWeekLastMonth}）`
        dataSource.value = saleDetailVOList;
        console.log(dataSource.value);

        columns[0].children[0].customRender = ({ text, index }) => {
          return {
            props: {
              rowSpan: getRowSpan1(index),
              colSpan: index === dataSource.value.length - 1 ? 3 : 1
            },
            children: h('span', {}, text)
          }
        };
        columns[0].children[1].customRender = ({ text, index }) => {
          return {
            props: {
              rowSpan: getRowSpan1(index),
              colSpan: index === dataSource.value.length - 1 ? 0 : 1
            },
            children: h('span', {}, text)
          }
        };
        columns[0].children[2].customRender = ({ text, index }) => {
          return {
            props: {
              colSpan: index === dataSource.value.length - 1 ? 0 : 1
            },
    ...
    onBeforeMount(() => {
      ...
      const { funcCode } = meta;
      pageCode.value = funcCode as number;
    });
